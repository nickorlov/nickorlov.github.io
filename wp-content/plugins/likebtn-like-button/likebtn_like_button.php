...
// Plugin version
define('LIKEBTN_VERSION', '2.3.0');
// Current DB version
define('LIKEBTN_DB_VERSION', 14);
...
function likebtn_votes_query($query_limit = '', $query_select = '', $query_orderby = '', $query_where = '')
{
    ...
    if ($votes_blog_id && $votes_blog_id != 1 && $votes_blog_id != 'all') {
        ...
        foreach ($blog_list as $blog) {
            ...
            if (!$query_select) {
                $query_select = $blog->blog_id . ' as blog_id, v.id, v.identifier, v.type, v.user_id, v.ip, v.created_at, i.ID as item_id, i.url ';
            }
            $query_list[] = $wpdb->prepare(_likebtn_get_votes_sql($prefix, $query_where, '', '', $query_select, $query_join), $query_parameters);
        }
        $query_prepared = ' SELECT SQL_CALC_FOUND_ROWS * from (' . implode(' UNION ', $query_list) . ") query {$query_orderby} {$query_limit} ";
    } else {
        if (!$query_select) {
    ...
    return array($query_prepared, $blogs, $votes_blog_id, $entity_name, $post_id, $user_id, $ip, $vote_type);
}
